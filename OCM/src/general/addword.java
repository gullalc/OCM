...
public class addword extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String meaning = request.getParameter("text_mean");
		...
		try
		{
			...
			String query = " Select word from wrd where word like  '"+word+"'";
			ResultSet rs = stmt.executeQuery(query);
			if(rs.next())
			{
				out.println("The word is already in the database");
			}
			else
			{
				int i = stmt.executeUpdate("Insert into wrd (word,meaning) values('"+word+"','"+meaning+"')");
				if(i>0)
				{
					out.println("Word submitted successfully");
				}
				else
				{
